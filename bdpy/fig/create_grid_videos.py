...
def draw_group_image_set(
    images,
    labels,
    font_colors,
    background_color=(255, 255, 255),
    image_size=(160, 160),
    image_margin=(1, 1, 0, 0),
    group_margin=(20, 0, 20, 0),
    max_column_size=13,
    title_fontsize=20,
    title_top_padding=70,
    title_left_padding=15,
    font_family_path=None,
    id_show=False,
    id_fontcolor="black",
    id_fontsize=18,
    image_id_list=[],
    ...
    size_x = (image_size[0] + image_margin[0] + image_margin[2]) * nImg_row + (
        group_margin[0] + group_margin[2]
    ) * turn_num
    size_y = (image_size[1] + image_margin[1] + image_margin[3]) * nImg_col + (
        group_margin[1] + group_margin[3]
    )
    ...
    for cind, (image_list, title) in enumerate(zip(images, labels)):
        for tind in range(total_image_size):
            ...
            x = (
                ...
                + group_margin[0]
                + row_index
                * (image_size[0] + image_margin[0] + image_margin[2])
            )
            ...
            y = (
                ...
                + group_margin[3]
                + column_index
                * (image_size[1] + image_margin[1] + image_margin[3])
            )
            image[
    ...
    draw.font = PIL.ImageFont.truetype(
        font=font_family_path, size=title_fontsize
    )
    ...
    if id_show:
        draw.font = PIL.ImageFont.truetype(
            font=font_family_path, size=id_fontsize
        )
        for tind in range(total_image_size):
            #  Calc text position
...
def load_inputs_from_one_source(
    inputs_list, is_image, is_mat, width=224, height=224
):
    """
    ...
    for target_file in inputs_list:
        if is_image and not is_mat:
            ...
            target_image = np.clip(
                loadmat(str(target_file))["raw_final_image_array"], 0, 255
            ).astype("uint8")[:, :, ::-1]
            target_image = expand2square(target_image)
            target_image = cv2.resize(target_image, dsize=(width, height))
            output.append(target_image)
        else:
            target_frames = load_one_movie_as_list(
                target_file, width=width, height=height
            )
            if len(target_frames) > maximum_length:
                maximum_length = len(target_frames)
            output.append(target_frames)
    if not is_image:
        tmp_list = []
        for frames in output:
            if len(frames) < maximum_length:
                tmp_frames = frames + [frames[-1]] * (
                    maximum_length - len(frames)
                )
...
def load_all_inputs(
    inputs_list,
    to_be_removed,
    reverse_exclusion_and_inclusion,
    index,
    n_targets,
    width=224,
    height=224,
    ...
    reverse_exclusion_and_inclusion: if True, to_be_removed is used as inclusion list
    ...
    for source_info in inputs_list:
        ...
        is_mat = (
            source_info["is_mat"]
            if "is_mat" in source_info and source_info["is_mat"]
            else False
        )
        ...
        # list up input files
        inputs_files = list(
            source_dir.glob(inputs_pattern.replace("<image_id>", "*"))
        )
        # sort
            ...
            ids = [
                int(
                    re.match(
                        str(source_dir)
                        + "/"
                        + inputs_pattern.replace("*", ".*").replace(
                            "<image_id>", "(.*)"
                        ),
                        str(path),
                    ).groups()[0]
                )
                for path in inputs_files
            ]
            # remove unfaborable image/movie
            if reverse_exclusion_and_inclusion:
                inputs_files = [
                    input_file
                    for (input_file, id) in zip(inputs_files, ids)
                    if id in to_be_removed
                ]
                ids = [id for id in ids if id in to_be_removed]
            else:
                inputs_files = [
                    input_file
                    for (input_file, id) in zip(inputs_files, ids)
                    if id not in to_be_removed
                ]
        ...
        if target_ids is None:
            ...
            assert target_ids == cur_target_ids, print(
                target_ids, cur_target_ids
            )

        inputs_files = inputs_files[
            n_targets * index : n_targets * (index + 1)
        ]

        # load data
        loaded = load_inputs_from_one_source(
            inputs_files, is_image, is_mat, width=width, height=height
        )
    ...
    for output in outputs:
        if len(output) < maximum_length:
            tmp_output = list(output) + [output[-1]] * (
                maximum_length - len(output)
            )
    ...
    for removal_targets in removal_conf.values():
        for removal_target in removal_targets:
            ...
            try:
                ...
                out_list.append(removal_target_base_path.stem)
    return out_list
...
def main():
    ...
    parser.add_argument(
        "--conf_path", type=str, required=True, help="configuration yaml path"
    )
    ...
    reverse_exclusion_and_inclusion = (
        "reverse_exclusion_and_inclusion" in conf
        and conf["reverse_exclusion_and_inclusion"]
    )
    ...
    for index in conf["index"]:
        inputs_array_list, tags, font_colors = load_all_inputs(
            inputs_list,
            to_be_removed_id_list,
            reverse_exclusion_and_inclusion,
            index,
            conf["n_targets"],
            width=width,
        ...
        output_filename = pathlib.Path(
            conf["output_filename"].replace("<index>", str(index))
        )
        output_path = output_dir / output_filename
        output_dir.mkdir(parents=True, exist_ok=True)

        if not output_path.suffix in [
            ".jpg",
            ".png",
            ".jpeg",
            ".JPEG",
            ...
            for i, one_frame_data in tqdm(enumerate(inputs_array_list)):
                frame_out = draw_group_image_set(
                    one_frame_data,
                    tags,
                    font_colors,
                    background_color=(255, 255, 255),
                    image_size=(height, width),
                    image_margin=conf["image_margin"],
                    group_margin=conf["group_margin"],
                    max_column_size=conf["max_column_size"],
                    title_fontsize=conf["title_fontsize"],
                    title_top_padding=conf["title_top_padding"],
                    title_left_padding=conf["title_left_padding"],
                    font_family_path=None,
                    id_show=False,
                    id_fontcolor="black",
                    id_fontsize=18,
